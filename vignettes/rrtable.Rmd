...
subtitle: "R package rrtable : Reproducible Research with R codes Table"
...
You can load the `rrtable` package with the folowing R command.
```{r}
require(rrtable)
```
...
Sample data `sampleData2` is included in rrtable package. You can see the sampleData2 by folllowing R command.
...
```{r}
df2flextable(sampleData2,align_body = "left")
```
...
knitr::include_graphics("https://raw.githubusercontent.com/cardiomoon/rrtable/master/man/figures/1.png")
```
